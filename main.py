    ...
    try:
        ...
        if os.path.exists(config_file):
            ...
            try:
                ...
                models["action_model"] = AutoModelForVideoClassification.from_pretrained(
                    'haipradana/s-h-o-p-domain-adaptation'
                ).to(device)
                models["image_processor"] = AutoImageProcessor.from_pretrained(
                    'haipradana/s-h-o-p-domain-adaptation'
                )
...
async def analyze_video(
    ...
    generate_video: bool = True  # Add parameter to control video generation
):
    ...
    try:
        ...
        if generate_video:
            ...
            result = await process_video_analysis(video_path, max_duration, generate_video=False)
...
async def process_video_analysis(video_path: str, max_duration: int = 30, generate_video: bool = False):
    ...
    for f_idx, result in enumerate(tracker):
        ...
        # Shelf detection
        res_shelf = models["shelf_model"](frame, device=device)
...
async def get_model_info():
    ...
    info = {
        ...
        "action_classes": models.get("id2label", {}),        "model_details": {
            ...
            "action_model": "haipradana/s-h-o-p-domain-adaptation (models/action_model/ or online)",
